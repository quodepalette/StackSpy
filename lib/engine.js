...
(function (g) {
  ...
  function evalTech(t, S) {
    ...
    // Bundlers erase package boundaries and often remove window globals. For well-known
    // browser libraries, a package/module signature inside a fetched bundle or source map
    // is a first-class fingerprint (the same general approach Wappalyzer uses for scripts
    // and JavaScript source). This fallback is intentionally conservative and only applies
    // to explicit package names or distinctive exported symbols.
    if (t.name === "Framer Motion" && !ev.some((e) => e.type === "body" || e.type === "sourceMaps" || e.type === "sourceNames" || e.type === "html")) {
      const hay = [S.body, S.sourceMaps, S.sourceNames, S.html, S.scripts, S.requests].join("\n");
      const m = hay.match(/(?:framer-motion|motion\/react|motion-dom|motion-utils|\bAnimatePresence\b|\bMotionConfig\b|\buseMotionValue\b|\bVisualElement\b)/i);
      if (m) add("bundle", "JavaScript bundle", m[0], 78, null);
    }
  ...
  function detect(facts, db, cats) {
    ...
    // Expand explicit technology dependencies (the same idea used by Wappalyzer's
    // "implies" relationships). This is important for bundled production apps: React,
    // for example, is normally not exposed as window.React by Next.js, even though the
    // framework is built on React. Keep implied detections visibly separate from direct
    // fingerprints by using a lower-confidence "implies" evidence family.
    const inferred = new Set();
    let changed = true;
    while (changed) {
      changed = false;
      for (const f of Array.from(found.values())) {
        if (f.conf < 55) continue;
        for (const dep of (f.tech.implies || [])) {
          if (found.has(dep) || inferred.has(dep)) continue;
          const depTech = db.find((x) => x.name === dep);
          if (!depTech) continue;
          const ev = [{
            type: "implies",
            label: "implied by " + f.tech.name,
            snippet: f.tech.name + " → " + dep,
            rawConfidence: Math.min(78, Math.max(62, f.conf - 10)),
            source: f.tech.name
          }];
          found.set(dep, {
            tech: depTech,
            conf: scoreEvidence(ev),
            ev,
            version: null,
            families: 1,
            inferred: true,
            inferredFrom: f.tech.name
          });
          inferred.add(dep);
          changed = true;
        }
    ...
    // Exclusions are applied after inference so an explicit exclude can suppress an
    // inferred dependency as well as a directly detected technology.
    for (const f of Array.from(found.values())) {
      for (const ex of f.tech.excludes || []) if (found.has(ex)) found.delete(ex);
    }

    const out = [];
    for (const f of found.values()) {
      const t = f.tech;
      const rawCategory = t.cat;
      const aliasMap = g.CATEGORY_ALIASES || {};
      const overrideMap = g.CATEGORY_OVERRIDES || {};
      const category = overrideMap[t.name] || aliasMap[rawCategory] || rawCategory;
      const meta = (g.CATEGORY_META || {})[category] || {};
      const cat = Object.assign({}, (cats || {})[rawCategory] || {}, meta);
      out.push({
        name: t.name, category, layer: cat.layer || "platform", website: t.website, description: t.d || "", tip: t.tip || "",
        version: f.version, confidence: f.conf, inferred: !!f.inferred, inferredFrom: f.inferredFrom || null, evidence: f.ev.slice(0, 8), evidenceFamilies: f.families || new Set(f.ev.map((e) => e.type)).size, evidenceQuality: f.inferred ? "inferred" : (f.conf >= 85 ? "strong" : f.conf >= 65 ? "good" : "corroborated"), details: extractDetails(t, S)
      });
    }
